...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const queued = await enqueueWeeklyChallengeGeneration(period.periodStart, true, true);
    await db.auditLog.create({
      data: {
        ...
        entityId: period.id,
        afterValue: { jobId: queued.job.id, reused: queued.reused, queueState: queued.state },
