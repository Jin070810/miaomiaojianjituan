...
type BirthdayProfile = { birthday: string | null; pendingBirthday: string | null; pendingEffectiveAt: string | null; visibleOnWall: boolean; onboardingSeenAt: string | null; nextSelfChangeAt: string | null };
...
type BirthdayMeData = { profile: BirthdayProfile | null; benefits: BirthdayBenefit[]; wishes: Array<{ id: string; benefitYear: number; presetCode: string; createdAt: string; sender: { id: string; nickname: string; avatarUrl: string | null } }>; presets: Array<{ code: string; label: string; message: string }> };
...
export default function BirthdayView({ onBack, onBalanceChanged }: { onBack: () => void; onBalanceChanged: () => void }) {
  ...
  async function load() {
    ...
    try {
      ...
      setShowBenefitsIntro(!nextMe.profile?.onboardingSeenAt);
  ...
  async function dismissBenefitsIntro() {
    setShowBenefitsIntro(false);
    try {
      const response = await fetch("/api/birthdays/onboarding", { method: "POST" });
      const result = await response.json();
      if (!response.ok) throw new Error(result.error ?? "生日权益引导保存失败");
      setMe((current) => current ? { ...current, profile: current.profile ? { ...current.profile, onboardingSeenAt: result.onboardingSeenAt } : current.profile } : current);
    } catch (failure) {
      setFeedback(failure instanceof Error ? failure.message : "生日权益引导保存失败");
    }
  }
  ...
  return <div className="member-content journal-page birthday-page">
    ...
    {showBenefitsIntro ? <BirthdayModal title="生日星愿权益" eyebrow="第一次见面" onClose={() => void dismissBenefitsIntro()}><div className="birthday-benefit-intro"><div><Sparkles size={20} /><span><strong>年度生日抽奖</strong><small>生日当天起开放 7 天，每年一次，必有奖品。</small></span></div><div><PartyPopper size={20} /><span><strong>生日作品加成</strong><small>生日当天投稿并通过，额外获得基础积分的 20%，年度最多 500 分。</small></span></div><div><Send size={20} /><span><strong>团友祝福与心意</strong><small>生日前后 7 天可收祝福卡，团友也可自愿赠送已有积分。</small></span></div><div><Star size={20} /><span><strong>寿星标识与年度纪念</strong><small>公开生日可点亮今日寿星标识，参与后获得生日星光勋章和私人纪念。</small></span></div></div><button className="journal-primary full-button" onClick={() => void dismissBenefitsIntro()}>我知道了</button></BirthdayModal> : null}
  </div>;
}
