...
生成提示词升级为 `weekly-challenge-v5-deterministic-targets`。服务端根据最近两周聚合数据确定整数基线、视频/点赞目标、难度和三个递增奖励阶段；DeepSeek 只润色匿名标题、说明与原因，不得返回任何业务数值。第一档 100 分，最终累计 300–1000 分，领取时只发放相对上次的差额。
...
3. 生成完成后核对状态为 `READY`、任务数等于上周提交成员数、所有任务类型均为 `COMBINED`、每个任务有 3 个递增阶段、最终奖励为 `300–1000`，且奖励策略版本为 `tiered-v2-hard-combined`。
4. 核对 `generationMode` 与 `fallbackBatchCount`。生产允许 `HYBRID` 或 `DETERMINISTIC` 按时发布，但必须产生 warning 告警并进入复盘；付费影子作为开关凭证时必须为纯 `AI` 且零降级。
5. 旧版本已生成但尚未生效的 `READY` 周期，必须在后台点击“按两周数据重新生成”。生成成功前旧任务不会激活，不可直接编辑数据库。
6. 周一 00:00 后执行生命周期巡检，确认周期变为 `ACTIVE` 且成员端可见任务。

若周期仍为 `FAILED`，说明失败发生在 audience、预算、事务或状态机等非 AI 发布依赖上。保持周挑战开关关闭，保留生成尝试、审计和告警记录，禁止直接改表。
...
- `DEEPSEEK_HARD_TIMEOUT_MS`：单次请求总时限，默认 180000。
- `DEEPSEEK_FIRST_BYTE_TIMEOUT_MS`：首响应时限，默认 60000。
- `DEEPSEEK_IDLE_TIMEOUT_MS`：SSE 连续无进度时限，默认 45000。
- `DEEPSEEK_MAX_OUTPUT_TOKENS`：单批输出上限，默认 6000。
...
Migration：`prisma/migrations/20260725120000_weekly_challenges/migration.sql`、`prisma/migrations/20260726130000_weekly_challenge_tiered_rewards/migration.sql`、`prisma/migrations/20260804170000_weekly_challenge_resilience/migration.sql`
...
- 韧性 migration 只新增生成来源/模式、降级摘要、运行 ID、输入哈希和检查点索引；不改写历史 migration。
...
- 300 人按 8 人分批完整生成；缺失、重复、非法字段或 provider 失败连续三次后使用确定性模板并完整发布。
- 生成中断后按输入哈希复用已校验批次，重试不重复调用 provider。
- BullMQ 上海时区周日 18:00 持久调度、队列去重、三次指数退避，以及维护巡检补入缺失/失败/过期租约任务。
...
4. 验证失败批次使用稳定模板且不会产生部分发布；管理员可查看生成模式、降级批次、来源和失败原因。
...
7. `/api/health` 基础设施正常时保持 `ok=true`；当前周期缺失、失败或降级时单独返回 `degraded=true` 和可操作原因。确认 scheduler 已配置且返回下一次触发时间。
...
- 首次上线或事故恢复时开关保持关闭；真实 DeepSeek、告警、双周期纯 AI 影子、数据库副本 migration 和 staging 验收完成后，由维护者明确确认才能开启。
- 发布顺序为备份校验、migration、同版本 Web/Worker、健康检查、staging 核心流程、观察队列和调度器；不得只更新 Web 或 Worker。
