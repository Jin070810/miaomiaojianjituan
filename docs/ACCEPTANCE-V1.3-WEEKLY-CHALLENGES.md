...
线上 `2026-07-27` 周期曾因模型给新人返回 `LIKE_SUM` 任务而失败。服务端的新人规则没有放宽；生成提示词现升级为 `weekly-challenge-v2`，为每名成员显式提供 `allowedTypes`，并明确视频/点赞目标的 null 语义。模型发生解析、覆盖或业务校验错误时，后续尝试会收到脱敏的上一条校验错误和修正要求，每次尝试保留真实提示词哈希。管理员重试失败周期时会同步更新周期记录中的模型和提示词版本。

发布后的恢复顺序：

1. 确认 App 与 Worker 是同一已审查 commit，`/api/health` 的数据库、Redis、Worker 和 DeepSeek 配置均为 `ok`。
2. 在后台“AI 周挑战”对失败周期点击“重试”；不得直接编辑数据库或删除失败周期。
3. 生成完成后核对状态为 `READY`、任务数等于冻结成员数、个人奖励总额不超过 10,000、`finalFailedBatches` 为空，且周期提示词版本为 `weekly-challenge-v2`。
4. 周一 00:00 后再执行生命周期巡检，确认周期变为 `ACTIVE` 且成员端可见任务。
