...
线上 `2026-07-27` 周期曾因模型给新人返回 `LIKE_SUM` 任务而失败。服务端的新人规则没有放宽；生成提示词现升级为 `weekly-challenge-v3-tiered-rewards`，为每名成员显式提供 `allowedTypes`，并明确视频/点赞目标的 null 语义。模型发生解析、覆盖或业务校验错误时，后续尝试会收到脱敏的上一条校验错误和修正要求，每次尝试保留真实提示词哈希。个人奖励由服务端按难度计算为三档累计奖励：第一档 100 分，最终累计 300–1000 分，领取时只发放相对上次的差额。
...
3. 生成完成后核对状态为 `READY`、任务数等于上周提交成员数、每个任务有 3 个递增阶段、最终奖励为 `300–1000`，`finalFailedBatches` 为空，且奖励策略版本为 `tiered-v1`。
4. 旧版本已生成但尚未生效的 `READY` 周期，必须在后台点击“升级阶梯奖励”。该操作会事务化移除上周无有效提交的成员、重算阶段与预算并写入审计日志；不可直接编辑数据库。
